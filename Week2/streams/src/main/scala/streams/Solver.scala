...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors.toStream.map(b => (b._1, b._2 :: history))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, history) #:: xs =>
      val toVisit = newNeighborsOnly(neighborsWithHistory(b, history), explored)
      (b, history) #:: from(xs ++ toVisit, explored + b)
  }
